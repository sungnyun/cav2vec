...
class AVHubertModel(BaseFairseqModel):
    ...
    def build_model(cls, cfg: AVHubertConfig, task: AVHubertPretrainingTask):
        ...
        rel_score = False
        if hasattr(cfg, 'rel_score'): # for pretraining
            rel_score = cfg.rel_score
        if hasattr(task, 'rel_score'): # for fine-tuning
            rel_score = task.rel_score
    ...
    def forward(
        ...
        if mask and self.masking_type == 'input':
            ...
            # occluded sequence indices
            if 'video_occ_indices' in source:
                occ_indices = source['video_occ_indices']
                mask_indices = torch.logical_or(mask_indices, occ_indices)
