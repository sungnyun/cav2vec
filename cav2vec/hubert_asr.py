...
class AVHubertSeq2SeqConfig(AVHubertAsrConfig):
    w2v_path_ours: str = field(
        default='', metadata={"help": "path to hubert asr model (pretrained)"}
    )
    ...
    freeze_encoder: bool = field(
        default=False, 
        metadata={'help': 'freeze encoder to train relscore first'},
    )
...
class AVHubertSeq2Seq(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, cfg, task):
        ...
        if cfg.get("rel_score", False):
            task_pretrain.rel_score = cfg.rel_score
        ...
        # freeze entire encoder (+front-end) but not scoring modules
        ...
        if cfg.w2v_path_ours:
            state_ours = checkpoint_utils.load_checkpoint_to_cpu(cfg.w2v_path_ours)
            state_ours_dict = state_ours['model']
            encoder_state_dict, decoder_state_dict = dict(), dict()
            for k, v in state_ours_dict.items():
                if 'encoder.w2v_model.' in k:
                    encoder_state_dict[k.replace('encoder.w2v_model.', '')] = v
                elif 'decoder.' in k:
                    decoder_state_dict[k.replace('decoder.', '')] = v
            del state_ours_dict

            encoder.w2v_model.load_state_dict(encoder_state_dict, strict=True)
