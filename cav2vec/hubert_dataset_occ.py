...
class AVHubertDataset(FairseqDataset):
    def __init__(
            ...
            noise_num=1,
            noise_chunk=False,
            visual_corruption_enabled=False, # change
            visual_corruption_prob=0.8,
            visual_corruption_max_freq=3,
            visual_occ_type='coco',
            visual_noise=False,
            output_clean=False,
    ):
        self.label_rates = (
        ...
        self.is_s2s = is_s2s
        self.visual_corruption_enabled = visual_corruption_enabled # change
        self.visual_corruption_prob = visual_corruption_prob
        self.visual_corruption_max_freq = visual_corruption_max_freq
        ...
        self.noise_prob_heavy = noise_prob_heavy
        self.noise_snr_heavy = noise_snr_heavy
        ...
        if self.visual_corruption_enabled: #change
            self.visual_corruption = Visual_Corruption_Modeling(occ_type=visual_occ_type)
    ...
    def load_feature(self, mix_name):
        ...
        if 'video' in self.modalities:
            video_feats, clean_video_feats, video_occ_indices = self.load_video(video_fn, output_clean=self.output_clean) # [T, H, W, 1]
        else:
            video_feats, clean_video_feats = None, None
            video_occ_indices = None
        if 'audio' in self.modalities:
            audio_fn = audio_fn.split(':')[0]
            sample_rate, wav_data = wavfile.read(audio_fn)
            occ_start = None
            if len(wav_data.shape) == 2: # Check if audio is stereo
                # Convert to mono by averaging the channels
                wav_data = wav_data.mean(axis=1).astype(wav_data.dtype)
            assert sample_rate == 16_000 and len(wav_data.shape) == 1
            if self.output_clean:
                clean_wav_data, clean_audio_feats = wav_data, None
            else:
                clean_wav_data, clean_audio_feats = None, None
            if np.random.rand() < self.noise_prob:
                wav_data, (occ_start, occ_end) = self.add_noise(wav_data)
        ...
        if audio_feats is not None:
            audio_occ_indices = np.full(audio_feats.shape[0], False)
            if occ_start is not None:
                audio_occ_indices[occ_start // 640 + 1:occ_end // 640 - 1] = True
        else:
            audio_occ_indices = None
        ...
        return video_feats, audio_feats, clean_video_feats, clean_audio_feats, video_occ_indices, audio_occ_indices
    ...
    def load_video(self, audio_name, output_clean=False):
        feats = custom_utils.load_video(os.path.join(self.audio_root, audio_name))
        clean_feats = copy.deepcopy(feats)
        occ_indices = np.full(feats.shape[0], False)

        if self.visual_corruption_enabled:
            lm_path = str(Path(audio_name)).replace('video', 'landmark_occ').replace('.mp4', '.pkl')
            ...
            if not os.path.exists(lm_path):
                ...
                lm = pkl['landmarks'] # need to debug
                yx_min = pkl['yx_min'] # need to debug

                if np.random.rand() < self.visual_corruption_prob:
                    freq1, freq2 = [np.random.choice(range(1, 1+self.visual_corruption_max_freq)) for _ in range(2)]
                    feats, _, occ_indices_1 = self.visual_corruption.occlude_sequence(feats, lm, yx_min, freq=freq1, return_indices=True)
                    occ_indices = occ_indices_1
                    if self.visual_noise:
                        feats, occ_indices_2 = self.visual_corruption.noise_sequence(feats, freq=freq2, return_indices=True)
                        occ_indices = np.logical_or(occ_indices_1, occ_indices_2)
                else:
                    pass
            
        if output_clean:
            feats, clean_feats = np.split(np.expand_dims(self.transform(np.concatenate((feats, clean_feats), axis=0)), axis=-1), 2)
        else:
            feats = np.expand_dims(self.transform(feats), axis=-1)
            clean_feats = None
        return feats, clean_feats, occ_indices
    ...
    def add_noise(self, clean_wav):
        ...
        noise_chunk = self.noise_chunk
        if randn < self.noise_prob_heavy:
            snr = self.noise_snr_heavy
            noise_chunk = True
        else:
        ...
        if noise_chunk:
            occ_len = np.random.randint(int(len(clean_wav) * 0.3), int(len(clean_wav) * 0.5)) #
            start_fr = np.random.randint(0, len(clean_wav) - occ_len)
            mixed = clean_wav
            mixed[start_fr:start_fr+occ_len] += adjusted_noise_wav[start_fr:start_fr+occ_len]
            occ_start, occ_end = start_fr, start_fr + occ_len
        else:
            mixed = clean_wav + adjusted_noise_wav
            occ_start, occ_end = None, None
        ...
        return mixed, (occ_start, occ_end)

    def __getitem__(self, index):
        video_feats, audio_feats, clean_video_feats, clean_audio_feats, video_occ_indices, audio_occ_indices = self.load_feature(self.names[index])
        ...
        clean_audio_feats, clean_video_feats = torch.from_numpy(clean_audio_feats.astype(np.float32)) if clean_audio_feats is not None else None, torch.from_numpy(clean_video_feats.astype(np.float32)) if clean_video_feats is not None else None
        video_occ_indices, audio_occ_indices = torch.from_numpy(video_occ_indices) if video_occ_indices is not None else None, torch.from_numpy(audio_occ_indices) if audio_occ_indices is not None else None
        ...
        return {
            ...
            "clean_video_source": clean_video_feats,
            "clean_audio_source": clean_audio_feats, 
            "label_list": labels, 
            "video_occ_indices": video_occ_indices,
            "audio_occ_indices": audio_occ_indices,
        }
    ...
    def collater(self, samples):
        ...
        clean_audio_source, clean_video_source = [s["clean_audio_source"] for s in samples], [s["clean_video_source"] for s in samples]
        video_occ_indices, audio_occ_indices = [s["video_occ_indices"] for s in samples], [s["audio_occ_indices"] for s in samples]
        ...
        if clean_video_source[0] is None:
            clean_video_source = None
        ...
        if audio_source is not None:
            collated_audios, padding_mask, audio_starts, collated_audio_occ_indices = self.collater_audio(audio_source, audio_size, occ_indices=audio_occ_indices)
        else:
            collated_audios, audio_starts = None, None
            collated_audio_occ_indices = None
        if clean_audio_source is not None:
            collated_clean_audios, _, _ = self.collater_audio(clean_audio_source, audio_size, audio_starts)
        else:
            collated_clean_audios = None
        if video_source is not None:
            collated_videos, padding_mask, audio_starts, collated_video_occ_indices = self.collater_audio(video_source, audio_size, audio_starts, occ_indices=video_occ_indices)
        else:
            collated_videos = None
            collated_video_occ_indices = None
        if clean_video_source is not None:
            collated_clean_videos, _, _ = self.collater_audio(clean_video_source, audio_size, audio_starts)
        else:
            collated_clean_videos = None
        targets_by_label = [
        ...
        source = {
            ...
            "clean_audio": collated_clean_audios,
            "clean_video": collated_clean_videos,
            "video_occ_indices": collated_video_occ_indices,
            "audio_occ_indices": collated_audio_occ_indices,
        }
    ...
    def collater_audio(self, audios, audio_size, audio_starts=None, occ_indices=None):
        audio_feat_shape = list(audios[0].shape[1:])
        collated_audios = audios[0].new_zeros([len(audios), audio_size]+audio_feat_shape)
        collated_occ_indices = torch.BoolTensor(len(audios), audio_size).fill_(False)
        padding_mask = (
        ...
        for i, audio in enumerate(audios):
            ...
            if diff == 0:
                ...
                if occ_indices is not None:
                    collated_occ_indices[i] = occ_indices[i]
            elif diff < 0:
                ...
                if occ_indices is not None:
                    collated_occ_indices[i, :diff] = occ_indices[i]
            else:
                collated_audios[i], audio_starts[i] = self.crop_to_max_size(
                    audio, audio_size, audio_starts[i] if start_known else None
                )
                if occ_indices is not None:
                    collated_occ_indices[i], _ = self.crop_to_max_size(
                        occ_indices[i], audio_size, audio_starts[i]
                    )
        ...
        if occ_indices is not None:
            return collated_audios, padding_mask, audio_starts, collated_occ_indices
        else:
