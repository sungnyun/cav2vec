...
if DBG:
    from hubert_dataset_occ import AVHubertDataset # change
...
class AVHubertPretrainingConfig(FairseqDataclass):
    ...
    noise_chunk: bool = field(default=False, metadata={"help": "whether to use audio noise as chunk (0.3~0.5 length)"})
    noise_prob_heavy: float = field(default=0, metadata={'help': 'use heavy noise in audio'})
    noise_snr_heavy: int = field(default=-10, metadata={'help': 'heavy noise SNR in audio'})
    fine_tuning: bool = field(default=False, metadata={"help": "set to true if fine-tuning"})
    visual_corruption_enabled: bool = field(default=False, metadata={'help': 'using visual occlusion'}) # change
    visual_corruption_prob: float = field(default=0.8, metadata={'help': 'visual corruption probability'})
    visual_corruption_max_freq: int = field(default=3, metadata={'help': 'maximum number of corruption frequency (>=1)'})
    visual_occ_type: str = field(default='coco', metadata={'help': 'visual occlusion type, choose between {coco | hands | pixelate}'})
    visual_noise: bool = field(default=True, metadata={'help': 'using visual noise'})
    output_clean: bool = field(default=False, metadata={"help": "additionally return clean wav/video data"})
...
class AVHubertPretrainingTask(FairseqTask):
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        self.datasets[split] = AVHubertDataset(
            ...
            noise_prob_heavy=self.cfg.noise_prob_heavy,
            noise_snr_heavy=self.cfg.noise_snr_heavy,
            noise_num=noise_num,
            noise_chunk=self.cfg.noise_chunk,
            visual_corruption_enabled=self.cfg.visual_corruption_enabled, # change
            visual_corruption_prob=self.cfg.visual_corruption_prob,
            visual_corruption_max_freq=self.cfg.visual_corruption_max_freq,
            visual_occ_type=self.cfg.visual_occ_type,
            visual_noise=self.cfg.visual_noise,
            output_clean=self.cfg.output_clean,
        )
