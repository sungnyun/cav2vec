...
def linear_interpolate(landmarks, start_idx, stop_idx): # check
...
# -- Face Transformation
def warp_img(src, dst, img, std_size): # check
...
def apply_transform(transform, img, std_size): # check
    warped = tf.warp(img, inverse_map=transform.inverse, output_shape=std_size)
    warped = warped * 255  # note output from warp is double image (value range [0,1])
    warped = warped.astype('uint8')
    return warped

def get_frame_count(filename): # check
    cap = cv2.VideoCapture(filename)
    total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    cap.release()
    return total

def read_video(filename): # check
    cap = cv2.VideoCapture(filename)
    while(cap.isOpened()):                                                 
...
def cut_patch(img, landmarks, height, width, threshold=5):
    ...
    y_min = int(round(center_y) - round(height)) # change
    x_min = int(round(center_x) - round(width)) # change
...
def load_args(default_config=None):
    ...
    parser.add_argument('--lm-occ-save-direc', default=None, help='occlusion landmark directory')
    # -- mean face utils
...
def crop_patch(video_pathname, landmarks, mean_face_landmarks, stablePntsIDs, STD_SIZE, window_margin, start_idx, stop_idx, crop_height, crop_width):
    ...
    while True:
        ...
        if len(q_frame) == margin:
            ...
            tf_landmarks.append(trans_landmarks)
...
def landmarks_interpolate(landmarks): # check
...
if __name__ == '__main__':
    ...
    # -- mean face utils
    STD_SIZE = (256, 256)
    mean_face_landmarks = np.load(args.mean_face, allow_pickle=True) # error
    ...
    for filename_idx, filename in enumerate(tqdm(fids)):
        ...
        # -- crop
        # change
        sequence, yx_min, tf_landmarks  = crop_patch(video_pathname, preprocessed_landmarks, mean_face_landmarks, stablePntsIDs, STD_SIZE, window_margin=args.window_margin, start_idx=args.start_idx, stop_idx=args.stop_idx, crop_height=args.crop_height, crop_width=args.crop_width)
        ...
        # -- save landmark_occ
        lm_save_path = os.path.join(args.video_direc, 'landmark_occ', filename)
        ...
        with open(lm_save_path + '.pkl', "wb") as pkl_file:
            pickle.dump({'landmarks': tf_landmarks, 'yx_min': yx_min}, pkl_file)
